...
public class ProduitController {
 ...
 public static void addStock() {
	    Produit produit = getProductToUpdate(
	            findByNameContains(OutInPut.saisirTexte("Entrez le nom du produit pour ajouter du stock : ")));
...
//Création d'un menu qui permet diminuer le nombre de produit qu'il y a en stock
 ...
 public static void applicationPromotion() {
	    String nom = OutInPut.saisirTexte("Nom du produit pour la promotion :");
	    Produit p = getProductToUpdate(findByNameContains(nom));
	    if (p != null) {
	        double pourcentage = OutInPut.saisirDouble("Pourcentage de remise (ex: 10 pour 10%) :");
	        double nouveauPrix = p.getPrix() * (1 - (pourcentage / 100));
	        p.setPrix(nouveauPrix);
	        OutInPut.afficher("Promotion appliquée ! Nouveau prix : " + nouveauPrix + " €");
	    }
	}
